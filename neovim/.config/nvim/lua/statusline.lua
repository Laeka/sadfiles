...
local function render_diagnostics(bufnr)
  ...
  if #raw_diagnostics == 0 then return diagnostic_indicators.ok end
  ...
  for level, count in pairs(diagnostic_counts) do
    if count > 0 then result = string.format("%s %s %d", result, diagnostic_indicators[level], count) end
  end
...
local function mode_name(mode) return string.upper(mode_table[mode] or "V-Block") end
